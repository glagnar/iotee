...
np = neopixel.NeoPixel(machine.Pin(15), 7, bpp=4)

def set_colorw(r, g, b, w):
    for i in range(7):
        np[i] = (r, g, b, w)
    np.write()
...
def pulsew(r, g, b, w, delay):
    for intensity in range(10, 255, 5):  # Smaller step for smoother transition
        set_colorw(int(r*intensity/255), int(g*intensity/255), int(b*intensity/255), int(w*intensity/255))
        time.sleep_ms(delay)
